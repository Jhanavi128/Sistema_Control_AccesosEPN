...
public class AdminPanel extends JFrame {
    ...
    private BusinessLogic.FactoryBL<DataAccess.DTOs.UsuarioDTO> blUsuario;

    public AdminPanel() {
        try {
            blUsuario = new BusinessLogic.FactoryBL<>(DataAccess.DAOs.UsuarioDAO.class);
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        String[] columns = {
                ...
                "Estudiante",
                "Carrera",
                "Guardia (Código)", // Cambiado título
                "Resultado",
                "Fecha Creación",
                "Estado"
        };
        ...
        PatButton btnDeleteAll = new PatButton("Eliminar Historial");
        PatButton btnExit = new PatButton("Cerrar");

        btnRefresh.addActionListener(e -> loadData());
        btnDeleteAll.addActionListener(e -> deleteHistory());
        btnExit.addActionListener(e -> System.exit(0));

        panelButtons.add(btnRefresh);
        panelButtons.add(btnDeleteAll);
        ...
        // ---------------- TAB 2: GESTIÓN ESTUDIANTES ----------------
        tabbedPane.addTab("Gestión de Estudiantes", new StudentManagementPanel());

        // ---------------- TAB 3: GESTIÓN GUARDIAS ----------------
        tabbedPane.addTab("Gestión de Guardias", new GuardManagementPanel());
    ...
    private void loadData() {
        try {
            ...
            for (RegistroIngresoDTO r : registros) {
                ...
                String carrera = "N/A";
                String guardiaCodigo = "ID: " + r.getIdUsuario();
                ...
                // 3. Obtener código del guardia
                try {
                    DataAccess.DTOs.UsuarioDTO guardia = blUsuario.getBy(r.getIdUsuario());
                    if (guardia != null) {
                        guardiaCodigo = guardia.getCodigoUnico();
                    }
                } catch (Exception ex) {
                    // Ignore
                }

                // 4. Crear la fila con los datos combinados
                Object[] row = {
                        r.getIdRegistroIngreso(),
                        codigoUnico, // Extraído de Estudiante
                        nombreCompleto, // Extraído de Estudiante
                        carrera, // Extraído de Periodo
                        guardiaCodigo, // AHORA ES EL CÓDIGO DEL GUARDIA
                        r.getResultado(),
                        r.getFechaCreacion(),
                        r.getEstado()
                };
    ...
    private void deleteHistory() {
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            try {
                // LLamar a la BL para eliminar
                blRegistro.eliminarTodoHistorial();
                // Recargar tabla
                loadData();
                JOptionPane.showMessageDialog(this, "Historial eliminado correctamente.");
            } catch (AppException e) {
                JOptionPane.showMessageDialog(this, "Error al eliminar historial: " + e.getMessage());
            }
        }
    }
}